...
export function NotificationsProvider({ children }) {
    ...
    const clearNotificationTimers = useCallback((key) => {
    ...
    const addNotification = useCallback((title, message, type = NotificationType.Info, icon, iconFillOpacity) => {
        ...
        setNotifications(prev => {
            ...
            if (updatedNotifications.length > 7) {
                ...
                notificationsToHide.forEach(notif => {
                    ...
                    setNotifications(current => current.map(n => n.key === notif.key
                        ? { ...n, hiding: true }
                        : n));
        ...
        const hideTimer = setTimeout(() => {
            setNotifications(prev => prev.map(notif => notif.key === newNotification.key
                ? { ...notif, hiding: true }
                : notif));
        }, 5000);
