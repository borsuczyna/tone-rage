...
export default function NotificationsInterface() {
    ...
    useRageEvent('addNotification', useCallback((data) => {
        addNotification(data.title, data.message, data.type, data.icon, data.iconFillOpacity);
        playNotificationSound(data.type);
    }, []));
    return (<div className={styles.main} style={{ top }}>
            {notifications.map((notification) => (<Notification key={notification.key} data={notification}/>))}
        </div>);
}
