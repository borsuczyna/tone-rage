...
export default function TextureDebugInterface() {
    ...
    const handleGenerateTexture = async () => {
        ...
        try {
            const textureData = await TextureService.createMarkerTexture(icon, upperText, lowerText);
            if (textureData) {
            ...
            <div className={styles.panel}>
                ...
                <div className={styles.controls}>
                    <div className={styles.inputGroup}>
                        ...
                        <input type="text" value={upperText} onChange={(e) => setUpperText(e.target.value)} className={styles.input} placeholder="Enter upper text..."/>
                    </div>

                    <div className={styles.inputGroup}>
                        <label className={styles.label}>Lower Text:</label>
                        <input type="text" value={lowerText} onChange={(e) => setLowerText(e.target.value)} className={styles.input} placeholder="Enter lower text..."/>
                    </div>
                    ...
                    <button onClick={handleGenerateTexture} disabled={isGenerating} className={styles.generateButton}>
                        {isGenerating ? 'Generating...' : 'Generate Texture'}
                    </button>
                </div>

                {generatedTexture && (<div className={styles.preview}>
                        <h3 className={styles.previewTitle}>Generated Texture:</h3>
                        <div className={styles.textureContainer}>
                            <img src={generatedTexture} alt="Generated texture" className={styles.textureImage}/>
                        </div>
                    </div>)}
