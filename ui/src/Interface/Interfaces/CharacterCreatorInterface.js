...
import { getBestUndershirtsForTop, getDefaultAppearance, getRandomAppearance, randomizeClothes, randomizeDNA, randomizeEyes, randomizeFace, randomizeFacialHair, randomizeHair } from '@shared/Models/Character/Character';
...
export default function CharacterCreatorInterface() {
    ...
    const grabBoxRef = useRef(null);
    const [characterAppearance, _setCharacterAppearance] = useState(getDefaultAppearance());
    const [isSaving, setIsSaving] = useState(false);
    const [confirmationModalOpen, setConfirmationModalOpen] = useState(false);
    const [hidingInterface, setHidingInterface] = useState(false);
    const { addNotification } = useNotifications();
    const changeAppearance = (key, value, appearance) => {
    ...
    const setCharacterAppearance = (values) => {
    ...
    const updateFullCharacterAppearance = (appearance) => {
        _setCharacterAppearance(appearance);
        triggerEvent('characterCreator:updateAppearance', appearance);
    };
    ...
    const saveCharacter = async () => {
        ...
        try {
            const result = await fetchServerData('characterCreator:saveCharacter', characterAppearance);
    ...
    useRageEvent('dumpTopsData', (topsData) => {
    ...
    return (<div className={csx(styles.container, hiding && styles.hiding, hidingInterface && styles.hidingInterface)}>
            ...
            <RightMenu isTransitioning={isTransitioning}>
                <div className={styles.categoryContent}>
                    {displayedCategory == 0 && <DNACategory characterAppearance={characterAppearance} setCharacterAppearance={setCharacterAppearance} randomize={randomizeCategoryAppearance}/>}
                    {displayedCategory == 1 && <HairCategory characterAppearance={characterAppearance} setCharacterAppearance={setCharacterAppearance} randomize={randomizeCategoryAppearance}/>}
                    {displayedCategory == 2 && <FacialHairCategory characterAppearance={characterAppearance} setCharacterAppearance={setCharacterAppearance} randomize={randomizeCategoryAppearance}/>}
                    {displayedCategory == 3 && <EyesCategory characterAppearance={characterAppearance} setCharacterAppearance={setCharacterAppearance} randomize={randomizeCategoryAppearance}/>}
                    {displayedCategory == 4 && <FaceShapeCategory characterAppearance={characterAppearance} setCharacterAppearance={setCharacterAppearance} randomize={randomizeCategoryAppearance}/>}
                    {displayedCategory == 5 && <ClothesCategory characterAppearance={characterAppearance} setCharacterAppearance={setCharacterAppearance} randomize={randomizeCategoryAppearance}/>}
                </div>
            </RightMenu>

            <div className={styles.grabBox} ref={grabBoxRef}></div>
            
            <Toolbar characterAppearance={characterAppearance} randomizeAppearance={randomizeAppearance} onFullUpdate={updateFullCharacterAppearance} saveCharacter={() => setConfirmationModalOpen(true)} isSaving={isSaving}/>

            {confirmationModalOpen && (<ConfirmationModal onConfirm={saveCharacter} onCancel={() => setConfirmationModalOpen(false)}/>)}
        </div>);
}
