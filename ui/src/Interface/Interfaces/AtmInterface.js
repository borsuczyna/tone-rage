...
export default function AtmInterface() {
    ...
    const fetchAtmData = useCallback(async () => {
        try {
            setIsLoading(true);
            const data = await fetchServerData('atm:getData', {});
    ...
    const handleDeposit = async (amount) => {
        ...
        try {
            setIsLoading(true);
            const response = await fetchServerData('atm:deposit', { amount });
    ...
    const handleWithdraw = async (amount) => {
        ...
        try {
            setIsLoading(true);
            const response = await fetchServerData('atm:withdraw', { amount });
            if (response.success && response.bankMoney !== undefined && response.walletMoney !== undefined && response.logs) {
                setBankBalance(response.bankMoney);
                setWalletMoney(response.walletMoney);
                setTransactions(response.logs);
    ...
    const handleTransfer = async (targetUser, amount) => {
        if (isLoading)
            return false;
        try {
            setIsLoading(true);
            const response = await fetchServerData('atm:transfer', { targetUser, amount });
            if (response.success && response.bankMoney !== undefined && response.walletMoney !== undefined && response.logs) {
                setBankBalance(response.bankMoney);
                setWalletMoney(response.walletMoney);
                setTransactions(response.logs);
    ...
    return (<>
                ...
                <div className={styles.atmMachine}>
                    ...
                    <div className={styles.mainLayout}>
                        ...
                        <div className={styles.content}>
                            {activeTab === 'dashboard' ? (<DashboardTab bankBalance={bankBalance} recentTransactions={transactions} userId={userId} accountName={userInfo.username} walletBalance={walletMoney} onDepositClick={() => setIsDepositModalOpen(true)} onWithdrawClick={() => setIsWithdrawModalOpen(true)} onTransferClick={() => setIsTransferModalOpen(true)} setActiveTab={setActiveTab}/>) : activeTab === 'transactions' ? (<TransactionsTab transactions={transactions}/>) : (<div className={styles.comingSoon}>
            ...
            {/* Modals */}
            <DepositModal isOpen={isDepositModalOpen} onClose={() => setIsDepositModalOpen(false)} onDeposit={handleDeposit} isLoading={isLoading}/>
            <WithdrawModal isOpen={isWithdrawModalOpen} onClose={() => setIsWithdrawModalOpen(false)} onWithdraw={handleWithdraw} isLoading={isLoading}/>
            <TransferModal isOpen={isTransferModalOpen} onClose={() => setIsTransferModalOpen(false)} onTransfer={handleTransfer} isLoading={isLoading}/>
        </>);
}
