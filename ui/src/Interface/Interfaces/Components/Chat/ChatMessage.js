    ...
    return (<div className={styles.chatMessage}>
            ...
            <div className={styles.messageContent}>
                <div className={styles.usernameContainer}>
                    ...
                    {message.emblemas && message.emblemas.length > 0 && (<div className={styles.emblemas}>
                            {message.emblemas.map((emblema) => (<EmblemaElement key={emblema} emblema={emblema}/>))}
                        </div>)}
                </div>
            ...
            return (<RichText key={index} elements={typeof msg === 'string' ? [msg] : Array.isArray(msg) ? msg : [msg]} className={style}/>);
        })}
                </div>
            </div>
        </div>);
