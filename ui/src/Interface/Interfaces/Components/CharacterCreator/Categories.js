...
export default function Categories({ activeCategory, setActiveCategory, isTransitioning }) {
    ...
    return (<div className={styles.categories}>
            ...
            {[0, 1, 2, 3, 4, 5]
            .map(category => (<>
                        <CategoryItem key={category} category={category} active={activeCategory === category} setActiveCategory={() => { if (!isTransitioning)
            setActiveCategory(category); }}/>
                        <CategoryItem key={category + 10} category={category} active={activeCategory === category} setActiveCategory={() => { if (!isTransitioning)
            setActiveCategory(category); }} activeItemGhost={true}/>
                    </>))}
        </div>);
}
