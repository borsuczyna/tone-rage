import { MoneyLogTypeColors, MoneyLogTypeColorsLight, MoneyLogTypeIcons, MoneyLogTypeNames } from '@shared/Models/MoneyLogData';
...
export default function TransactionTableItem({ transaction }) {
    ...
    const IconElement = Icons[actionIcon.charAt(0).toUpperCase() + actionIcon.slice(1).replace(/-([a-z])/g, (g) => g[1].toUpperCase())] || Icons['Bell'];
    return (<tr className={styles.transactionsTableRow}>
            <td className={styles.transactionDescription}>
                <div className={styles.transactionTableAction} style={{ '--icon-color': actionColor, '--text-color': actionTextColor }}>
