...
export default function Map({ image = null, defaultZoom = 1, defaultPosition = { x: 0, y: 0 }, borders = {}, blips = [], mapSize = DEFAULT_MAP_SIZE, blipSize = DEFAULT_BLIP_SIZE, zoomSensitivity = DEFAULT_ZOOM_SENSITIVITY }) {
    ...
    // Default borders
    const { minZoom = 0.5, maxZoom = 5, minX = -Infinity, maxX = Infinity, minY = -Infinity, maxY = Infinity } = borders;
    ...
    return (<div ref={containerRef} className={styles.mapContainer} onMouseDown={handleMouseDown}>
			<img ref={imageRef} src={image || DEFAULT_IMAGE} alt="Map" className={styles.mapImage} style={{
            ...
            const IconComponent = Icons[blip.icon] || Icons.MapPin;
            return (<div key={blipKey} className={styles.blip} style={{
                    left: `${blipX}px`,
                    top: `${blipY}px`,
                    ...
                    borderRadius: '50%'
						...
						<IconComponent size={blipPixelSize * 0.6} color='#ffffff' className={styles.blipIcon} title={blip.label}/>
