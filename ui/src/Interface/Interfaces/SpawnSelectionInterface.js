...
import { spawnData } from '@shared/SpawnsData';
...
export default function SpawnSelectionInterface() {
    ...
    const handleSpawnSelect = (categoryId, locationId) => {
    ...
    const handleConfirmSpawn = async () => {
        ...
        const response = await fetchServerData('spawn:select', selectedSpawn);
        if (!response.success) {
    ...
    return (<div className={csx(styles.container, isDisappearing && styles.disappearing)}>
            <div className={styles.spawnPanel}>
                ...
                <div className={styles.categoryList}>
                    {spawnData.map((category, index) => (<div key={index} className={styles.categoryItem}>
                            <button className={styles.categoryButton} onClick={() => toggleCategory(index)}>
                                <div className={styles.categoryHeader}>
                                    <Icons.MapPin size='1.3rem'/>
                                    <span>{category.name}</span>
                                </div>
                                <Icons.ChevronDown size='1.3rem' className={`${styles.chevron} ${expandedCategories.has(index) ? styles.expanded : ''}`}/>
                            </button>

                            <div className={`${styles.locationListWrapper} ${expandedCategories.has(index) ? styles.expanded : ''}`}>
                                <div className={styles.locationList}>
                                    {category.locations.map((location, locationIndex) => (<button key={locationIndex} className={`${styles.locationButton} ${selectedSpawn?.[0] === index && selectedSpawn?.[1] === locationIndex ? styles.selected : ''}`} onClick={() => handleSpawnSelect(index, locationIndex)}>
                                            <Icons.MapPin size='1rem'/>
                                            <span>{location.name}</span>
                                            {selectedSpawn?.[0] === index && selectedSpawn?.[1] === locationIndex && (<Icons.Check size='1rem' className={styles.checkIcon}/>)}
                                        </button>))}
                                </div>
                            </div>
                        </div>))}
                ...
                <div className={styles.actions}>
                    <Button variant="primary" onClick={handleConfirmSpawn} disabled={!selectedSpawn} loading={isLoading} style={{ width: '100%' }}>
