...
export default function HandlingEditorInterface() {
    ...
    const [expandedCategories, setExpandedCategories] = useState(new Set(['Physical']));
    const [hasChanges, setHasChanges] = useState(false);
    useRageEvent('setHandlingData', useCallback((data) => {
    ...
    return (<div className={styles.container}>
			<div className={styles.panel}>
				...
				<div className={styles.content}>
					{Object.entries(handlingCategories).map(([category, fields]) => (<div key={category} className={styles.categorySection}>
							<button className={styles.categoryHeader} onClick={() => toggleCategory(category)}>
								<div className={styles.categoryTitle}>
									<Icons.ChevronDown size="1.2rem" className={`${styles.chevron} ${expandedCategories.has(category) ? styles.expanded : ''}`}/>
									<span>{category}</span>
								</div>
							</button>

							{expandedCategories.has(category) && (<div className={styles.fieldsGrid}>
									{fields.map((field) => (<div key={field.key} className={styles.field}>
											<label className={styles.fieldLabel}>{field.label}</label>
											<div className={styles.fieldControl}>
												<input type="number" className={styles.fieldInput} value={handlingData[field.key] || 0} onChange={(e) => handleValueChange(field.key, parseFloat(e.target.value) || 0)} step={field.step} min={field.min} max={field.max}/>
												<input type="range" className={styles.fieldSlider} value={handlingData[field.key] || 0} onChange={(e) => handleValueChange(field.key, parseFloat(e.target.value) || 0)} step={field.step} min={field.min} max={field.max}/>
											</div>
										</div>))}
								</div>)}
						</div>))}
