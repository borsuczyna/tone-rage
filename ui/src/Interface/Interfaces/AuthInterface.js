...
export default function AuthInterface() {
    ...
    useRageEvent('credentials:loaded', useCallback((credentials) => {
    ...
    const handleLogin = async () => {
        ...
        setIsLoading(true);
        try {
            let response = await fetchServerData('auth:login', loginData);
            if (response?.success) {
    ...
    const handleRegister = async () => {
        ...
        setIsLoading(true);
        try {
            let response = await fetchServerData('auth:register', registerData);
            if (response?.success) {
    ...
    if (!isInterfaceVisible('AuthInterface'))
        return null;
    return (<div className={`${styles.container} ${isDisappearing ? styles.disappearing : ''}`}>
            <div className={styles.contentWrapper}>
                <div className={styles.authPanel}>
                    {currentPage === 'login' && (<LoginForm loginData={loginData} setLoginData={setLoginData} onLogin={handleLogin} onSwitchToRegister={() => setCurrentPage('register')} isLoading={isLoading}/>)}

                    {currentPage === 'register' && (<RegisterForm registerData={registerData} setRegisterData={setRegisterData} onRegister={handleRegister} onSwitchToLogin={() => setCurrentPage('login')} onShowRules={() => setCurrentPage('rules')} isLoading={isLoading}/>)}

                    {currentPage === 'rules' && (<RulesForm onBackToLogin={() => setCurrentPage('login')} onAcceptAndRegister={() => setCurrentPage('register')}/>)}
                </div>
