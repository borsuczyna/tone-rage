...
export default function ScoreboardInterface() {
    ...
    const IconComponent = Icons[playerCategories[selectedCategoryIndex].icon];
    const nextCategory = () => {
    ...
    const fetchScoreboardData = async () => {
        try {
            const data = await fetchClientData('scoreboard:getData', null);
            setPlayers(data);
        }
        catch (error) {
    ...
    return (<div className={styles.container}>
            <div className={csx(styles.scoreboard, hiding && styles.hiding)}>
                <div className={styles.header}>
                    ...
                    <InputField value={searchTerm} onChange={(e) => setSearchTerm(e)} placeholder={translate('scoreboard.search')} rightElement={(<div className={styles.categorySelector} onClick={nextCategory}>
                ...
                <div className={styles.playersList}>
                    {filteredPlayers.length > 0 ? <>
                        {filteredPlayers.map((player, index) => (<div key={index} className={styles.playerCard}>
                                ...
                                <div className={styles.playerInfo}>
                                    <div className={styles.playerName}>
                                        ...
                                        <div className={styles.emblemasContainer}>
                                            {player.emblemas?.map((emblema) => (<EmblemaElement key={emblema} emblema={emblema}/>))}
                                        </div>
                                    </div>
