...
export default function TopsMakerInterface() {
    ...
    const [activeUndershirt, setActiveUndershirt] = useState(null);
    const [activeTorso, setActiveTorso] = useState(null);
    const [availableUndershirts, setAvailableUndershirts] = useState([]);
    ...
    const _setActiveUndershirt = (id) => {
        setActiveUndershirt(id);
        triggerEvent('TopsMakerInterface:SetActiveUndershirt', { itemId: id });
    };
    const _setActiveTorso = (id) => {
        setActiveTorso(id);
        triggerEvent('TopsMakerInterface:SetActiveTorso', { itemId: id });
    };
    return (<div className={styles.container}>
            <div className={styles.undershirtsContainer}>
                {undershirts.map((id) => (<div key={id} className={csx(styles.undershirtItem, activeUndershirt === id && styles.active, availableUndershirts.includes(id) && styles.available)} onClick={() => _setActiveUndershirt(id)}>
                        <img src={path.replace('{cid}', '8').replace('{id}', id.toString())} alt={`Undershirt ${id}`}/>
                    </div>))}
            </div>

            <div className={styles.undershirtsContainer}>
                {maleTorsos.map((id) => (<div key={id} className={csx(styles.undershirtItem, activeTorso === id && styles.active)} onClick={() => _setActiveTorso(id)}>
