...
export default class AudioService {
    private static loadedSounds: Map<string, Howl> = new Map();
    private static fadeIntervals: Map<Howl, number> = new Map();
     ...
     */
    static async playSound(src: string, options: Partial<{ 
        volume: number; 
        loop: boolean; 
        cache: boolean; 
        onend?: () => void 
     ...
     */
    static preloadSounds(sounds: string[]): void {
        sounds.forEach(src => {
     ...
     */
    static unloadSound(src: string): void {
     ...
     */
    static fadeIn(sound: Howl, targetVolume: number = 1, duration: number = 2000): void {
        if (!sound) return;
     ...
     */
    static fadeOut(sound: Howl, duration: number = 2000, stopAfterFade: boolean = true): void {
        if (!sound) return;
     ...
     */
    private static clearFade(sound: Howl): void {
     ...
     * @returns Promise that resolves when sound starts playing
     */
    static async playSoundWithFade(src: string, options: Partial<{ 
        volume: number; 
        loop: boolean; 
        cache: boolean; 
        onend?: () => void;
