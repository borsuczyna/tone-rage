export default class DrawingService {
     ...
     */
    static async waitForImageToLoad(htmlImageElement) {
     ...
     */
    static async loadImage(src, crossOrigin = 'anonymous') {
     ...
     */
    static drawText(context, text, x, y, alignX = 'left', alignY = 'alphabetic', font = 'Inter, Arial, sans-serif', fontSize = '16px', bold = false, color = 'white', shadow = null) {
     ...
     */
    static drawImage(context, image, x, y, w, h, color) {
        if (color) {
     ...
     */
    static drawCircularImage(context, image, x, y, radius) {
        context.save();
     ...
     */
    static measureText(context, text, font, fontSize, bold = false) {
        context.font = `${bold ? 'bold ' : ''}${fontSize} ${font}`;
     ...
     * Draws a circle
     */
    static drawCircle(context, x, y, radius, color, fill = true) {
        context.save();
     ...
     */
    static async drawLucideReactIcon(ctx, _icon, // Underscore prefix to indicate intentionally unused
    x, y, width, height, strokeRgba, fillRgba) {
