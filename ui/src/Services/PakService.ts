const pakCache = new Map<string, any>();
const urlCache = new Map<string, string>();

async function loadPak(pakUrl: string) {
    if (pakCache.has(pakUrl)) return pakCache.get(pakUrl);
...
export function pakImage(pakUrl: string, fileName: string) {
